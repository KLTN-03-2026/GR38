  ...
  try {
    ...
    if (existingReport) {
      return res.status(400).json({
        success: false,
        message:
          "Bạn đã gửi báo cáo cho nội dung/người này rồi và đang chờ Admin xử lý.",
      });
    }
    ...
    if (error.name === "CastError" && error.kind === "ObjectId") {
      return res
        .status(400)
        .json({ success: false, message: "ID không đúng định dạng hợp lệ." });
    }
...
export const updateReportStatus = async (req, res) => {
  try {
    ...
    const resolvedAt =
      status === REPORT_STATUS.RESOLVED || status === REPORT_STATUS.REJECTED
        ? Date.now()
        : null;
