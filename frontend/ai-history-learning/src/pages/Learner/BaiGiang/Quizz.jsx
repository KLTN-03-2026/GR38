...
import { 
  Loader2, AlertCircle, ChevronLeft, ChevronRight, Trophy, 
  CheckCircle2, XCircle, HelpCircle, BookOpen, ArrowLeft, MoreHorizontal 
} from "lucide-react";
...
const Quizz = ({ lessonId, lectureTitle }) => {
  ...
  if (!selectedQuiz) {
    return (
      <div className="p-4">
        <div className="mb-6">
          <h2 className="text-xl font-black text-gray-800 uppercase tracking-tight">Bài tập ôn tập</h2>
          <p className="text-xs text-gray-400 font-medium italic truncate">{lectureTitle}</p>
        </div>
        ...
        {quizzesList.length > 0 ? (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
              {currentQuizzes.map((quiz) => (
                <div key={quiz._id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all group">
                  <div className="aspect-video bg-orange-50 rounded-xl mb-3 flex items-center justify-center overflow-hidden">
                    <BookOpen size={28} className="text-orange-200 group-hover:scale-110 transition-transform" />
                  </div>
                  <h3 className="font-bold text-gray-800 text-[13px] line-clamp-1 uppercase mb-2">{quiz.title}</h3>
                    ...
                    if (totalPages > 5) {
                      if (pageNum > 2 && pageNum < totalPages - 1 && Math.abs(pageNum - currentPage) > 1) {
                        if (pageNum === 3 || pageNum === totalPages - 1) {
                          return <MoreHorizontal key={`dots-${pageNum}`} size={14} className="mx-1 text-gray-300" />;
                        }
                        return null;
                      }
                    }
          ...
          <div className="text-center py-10 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50">
            ...
            <p className="text-gray-400 text-xs font-bold uppercase">Chưa có bài tập</p>
          </div>
        )}
      </div>
    );
  ...
  if (isSubmitted && result) return (
    <div className="p-8 bg-white rounded-2xl text-center shadow-sm border border-gray-100">
      ...
      <h2 className="text-xl font-black text-gray-800 mb-1 tracking-tight">HOÀN THÀNH</h2>
      <p className="text-gray-400 mb-6 text-sm font-medium">{selectedQuiz.title}</p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <div className="p-4 bg-green-50 rounded-2xl border border-green-100 flex flex-col items-center">
          ...
          <p className="text-[9px] text-green-600 font-bold uppercase tracking-widest">Đúng</p>
        </div>
        <div className="p-4 bg-red-50 rounded-2xl border border-red-100 flex flex-col items-center">
          ...
          <p className="text-2xl font-black text-red-600">{result.incorrectCount}</p>
          <p className="text-[9px] text-red-600 font-bold uppercase tracking-widest">Sai</p>
        </div>
      </div>
  ...
  return (
    <div className="p-4 bg-white rounded-xl">
      ...
      >
        ...
        <span className="text-[10px] font-black uppercase tracking-tighter">Thoát bài làm</span>
      </button>

      <div className="mb-6">
        <div className="flex justify-between items-end mb-2">
          <span className="text-[#f26739] font-black text-xl leading-none">
            {currentQuestionIndex + 1}<span className="text-gray-300 text-[10px] font-normal ml-0.5">/{questions.length}</span>
          </span>
          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight line-clamp-1 max-w-[150px]">{selectedQuiz.title}</p>
        </div>
      ...
      <div className="bg-[#F9FAFB] p-6 rounded-[28px] border border-gray-100 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-2.5">
          {(currentQuestion?.options || []).map((opt, idx) => (
            ...
            >
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-3 text-[9px] font-bold ${
                selectedOption === opt ? "border-[#f26739] bg-[#f26739] text-white" : "border-gray-200 text-gray-400"
              }`}>
                {String.fromCharCode(65 + idx)}
              </div>
              <span className={`text-[13px] font-semibold ${selectedOption === opt ? "text-orange-900" : "text-gray-700"}`}>
                {opt}
              </span>
            </button>
          ))}
        ...
        <div className="flex justify-between mt-8 pt-4 border-t border-gray-50">
          ...
          >
            {currentQuestionIndex === questions.length - 1 ? "Gửi kết quả" : "Câu tiếp"} 
            {currentQuestionIndex !== questions.length - 1 && <ChevronRight size={14} />}
          </button>
        </div>
      </div>
    </div>
