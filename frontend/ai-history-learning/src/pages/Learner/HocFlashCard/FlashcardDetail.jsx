...
const FlashcardDetail = () => {
  ...
  const { id } = useParams(); // Lấy ID từ URL để hiển thị tiêu đề động nếu cần
  ...
  const questions = [
    { 
      q: "Chiến tranh kháng chiến chống mỹ ai là người lãnh đạo cuộc chiến tranh kháng chiến chống mỹ giải phóng miền nam??", 
      a: "Chủ tịch Hồ Chí Minh là người lãnh đạo dành thắng lợi giải Phóng Miền Nam Việt Nam" 
    },
    { 
      q: "Chiến dịch Điện Biên Phủ diễn ra vào thời gian nào?", 
      a: "Từ ngày 13/3/1954 đến 7/5/1954." 
    },
    { 
      q: "Ai là tổng chỉ huy quân đội Việt Nam trong chiến dịch Điện Biên Phủ?", 
      a: "Đại tướng Võ Nguyên Giáp." 
    },
  ];

  // Hàm lấy ảnh từ assets (Sửa đường dẫn lùi 3 cấp)
  const getAssetUrl = (imageName) => {
    try {
      // Lùi 3 cấp: FlashcardDetail -> HocFlashCard -> Learner -> pages -> src/assets
      return new URL(`../../../assets/${imageName}`, import.meta.url).href;
    } catch (err) {
      return "https://via.placeholder.com/400x200?text=History+Image";
    }
  };
  ...
  return (
    <div className="flex flex-col items-start p-[20px_16px] gap-[20px] w-full min-h-screen bg-[#FAFAFA]">
      ...
      <div className="w-full bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div 
          ...
          className={`relative w-full min-h-[400px] rounded-3xl flex flex-col p-8 cursor-pointer transition-all duration-500 shadow-lg border-2 ${
            isFlipped 
              ? "bg-[#47ED70] border-[#36BA58] rotate-y-180" 
        ...
        <div className="flex justify-between items-center mt-10 px-4 text-[#71717A]">
          <div className="flex flex-col">
             <span className="font-bold text-[18px] text-[#18181B]">
                {currentIndex + 1} / {questions.length}
             </span>
             <span className="text-[13px] font-medium uppercase tracking-widest text-gray-400">Tiến độ câu</span>
          </div>
                ...
                {questions.map((_, i) => (
                    <button 
                      ...
                      className={`w-10 h-10 flex items-center justify-center rounded-xl font-bold transition-all ${
                        currentIndex === i 
                          ? "bg-[#18181B] text-white shadow-lg scale-110" 
