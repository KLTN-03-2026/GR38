...
const ConfirmGenerateModal = ({ type, onConfirm, onCancel, loading }) => (
  ...
  >
    ...
    >
      <div className={`w-11 h-11 mx-auto mb-4 flex items-center justify-center rounded-full ${
        type === "quiz" ? "bg-orange-50 border border-orange-100" : "bg-purple-50 border border-purple-100"
      }`}>
      ...
      <div className="flex gap-2">
        ...
        <button onClick={onConfirm} disabled={loading}
          className={`flex-1 py-2.5 rounded-xl text-sm text-white font-medium transition flex items-center justify-center gap-2 ${
            type === "quiz" ? "bg-[#F26739] hover:bg-orange-600" : "bg-purple-500 hover:bg-purple-600"
          } disabled:opacity-60`}>
          {loading ? <><IconSpinner /> Đang tạo...</> : "✨ Tạo ngay"}
        </button>
      </div>
    </div>
  </div>
...
export default function DocumentsDetailPage() {
  ...
  try {
    setQuizLoading(true);
    const res = await axios.get(`${API}/quizzes/${id}`, { headers: getAuthHeader() });
  ...
  const loadFlashList = async () => {
    try {
      ...
      const res = await axios.get(`${API}/flashcards?documentId=${id}`, { headers: getAuthHeader() });
      const raw = res.data.data ?? res.data ?? [];
  ...
  try {
    ...
    await axios.post(`${API}/ai/generate-quiz`, { documentId: id, numQuestions: 10 }, { headers: getAuthHeader() });
 ...
 const handleGenerateFlash = async () => {
  try {
    ...
    await axios.post(`${API}/ai/generate-flashcards`, { documentId: id, numQuestions: 10 }, { headers: getAuthHeader() });
  ...
  const coverImg = doc?.coverImage || DEFAULT_COVERS[0];
        ...
        {activeTab === "Thông tin" && (
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
              <img src={coverImg} alt={doc.title} className="w-full h-52 object-cover bg-gray-100" />
              <div className="p-4">
                ...
                <div className="flex gap-4 text-sm text-gray-500 flex-wrap">
                  <span className={`flex items-center gap-1.5 font-medium ${
                    ...
                    doc.status === "processing" ? "text-orange-500" : "text-red-500"
                  }`}>
                    {doc.status === "ready" ? "✓ Đã xử lý" :
                     doc.status === "processing" ? "⏳ Đang xử lý..." : "✗ Lỗi xử lý"}
                  </span>
              ...
              <div className="px-4 py-3 space-y-2.5 text-sm text-gray-600">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-medium ${
                    ...
                    {doc.status === "ready" ? "Đã xử lý" :
                     doc.status === "processing" ? "Đang xử lý..." : "Lỗi"}
                  </span>
                </div>
              </div>
            </div>
              ...
              {quizLoading ? (
                ...
                quizList.map((quiz, idx) => (
                  <div key={quiz._id ?? idx}
                    ...
                    onClick={() => navigate(`/teacher/quiz/${quiz._id}`, { state: { doc, quizList } })}>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {quiz.status && (
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                          quiz.status === "published" ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-500"
                        }`}>
