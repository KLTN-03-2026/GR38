...
const shuffleOpts = (q) => {
  ...
  return { ...q, options: s.map((o) => o.t), answer: s.findIndex((o) => o.isAns) };
};
    ...
    <div className="space-y-4">
          ...
          {q.options.map((opt, i) => (
            ...
            >
              <div className={`w-4.5 h-4.5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
                selected === i ? "border-blue-500 bg-blue-500" : "border-gray-300"
              }`}>
      ...
      <div>
        ...
        <div className="flex items-center justify-between gap-3">
          <button
            onClick={() => goTo(currentQ - 1, "right")} disabled={currentQ === 0}
        ...
        <div className="flex justify-between mt-2.5">
          <button onClick={() => { setCurrentQ(0); setAnswers({}); }} className="text-xs text-gray-400 hover:text-gray-600 transition">
            Làm lại từ đầu
          </button>
          <button onClick={handleSubmit} className="text-xs text-orange-500 hover:text-orange-600 font-medium transition">
            Nộp bài ngay →
          </button>
        </div>
      </div>
