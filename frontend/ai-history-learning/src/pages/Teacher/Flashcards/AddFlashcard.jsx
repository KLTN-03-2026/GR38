...
const AddFlashcard = () => {
  ...
  const removeCard = (i) => { if (cards.length > 1) setCards((p) => p.filter((_, idx) => idx !== i)); };
  const updateCard = (i, field, value) => {
    setCards((p) => p.map((c, idx) => idx === i ? { ...c, [field]: value } : c));
    setErrors((p) => { const n = { ...p }; delete n[`card_${i}_${field}`]; return n; });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formData.append("cards", JSON.stringify(cards.map((c) => ({ front: c.front.trim(), back: c.back.trim() }))));
      if (imagePreview) {
        const res  = await fetch(imagePreview);
        const blob = await res.blob();
        formData.append("thumbnail", blob, "thumbnail.jpg");
      }
      await api.post("/flashcards/manual", formData, { headers: { "Content-Type": "multipart/form-data" } });
      navigate("/teacher/flashcards");
    } catch (err) {
      setSaveErr(err?.response?.data?.message || "Tạo bộ thẻ thất bại, vui lòng thử lại.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="flex-1 bg-[#FAFAFA] min-h-screen flex flex-col items-center p-8 font-['Inter']">
      <div className="w-full max-w-[860px]">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-[24px] font-black text-[#18181B] uppercase tracking-tight">Tạo bộ Flashcard mới</h1>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm mb-6">
          ...
          <input
            ...
            className={`w-full h-[46px] px-4 rounded-xl border text-[15px] text-[#18181B] outline-none transition-colors ${
              errors.title ? "border-red-400 bg-red-50" : "border-gray-200 bg-[#FAFAFA] focus:border-[#F26739]"
            }`}
          />
          {errors.title && <p className="text-red-500 text-[12px] mt-1.5">{errors.title}</p>}
        </div>
        ...
        <CardTable
          ...
          updateCard={updateCard} removeCard={removeCard} addCard={addCard}
        />

        {/* Counter */}
        <div className={`flex items-center justify-between mb-5 px-1 ${errors.minCards ? "text-red-500" : "text-gray-400"}`}>
          <span className="text-[12px] font-semibold">
            {cards.length < MIN_CARDS ? `⚠️ ${errors.minCards || `Cần ít nhất ${MIN_CARDS} thẻ`}` : `✅ ${cards.length} thẻ — đủ điều kiện tạo bộ`}
          </span>
          <span className={`text-[12px] font-bold px-2.5 py-0.5 rounded-full ${cards.length >= MIN_CARDS ? "bg-green-50 text-green-500" : "bg-red-50 text-red-400"}`}>
        ...
        {saveErr && (
          <div className="mb-6 flex items-center gap-2.5 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
            <svg width="16" height="16" fill="none" stroke="#ef4444" viewBox="0 0 24 24" className="shrink-0">
        ...
        <div className="flex gap-4 justify-end">
          <button onClick={() => navigate("/teacher/flashcards")} disabled={saving}
          ...
          <button onClick={handleSave} disabled={saving}
            ...
            {saving ? <><Loader2 size={16} className="animate-spin" /> Đang lưu...</> : "Lưu bộ thẻ"}
          </button>
        </div>
