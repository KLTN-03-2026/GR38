        ...
        <div className="relative w-full rounded-xl overflow-hidden border border-gray-200 group" style={{ height: 160 }}>
          ...
          {name && (
            <div className="absolute bottom-2 left-2 bg-black/50 backdrop-blur-sm rounded-md px-2 py-0.5">
              <span className="text-[10px] text-white truncate max-w-[200px] block">{name}</span>
            </div>
          )}
        </div>
      ) : (
...
export default function AddQuizModal({ onClose, onSave, documentId, editQuiz }) {
  ...
  useEffect(() => {
    ...
    if (editQuiz.documentId) setSelectedDoc(editQuiz.documentId);
  ...
  const handleRemoveQ = (i) => { setQuestions((p) => p.filter((_, j) => j !== i)); setListErr(""); };
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      const data = {
        title: quizTitle, description, difficulty, timeLimit: Number(timeLimit),
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100 shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-gray-800">
              {isEdit ? (step === 1 ? "Chỉnh sửa Quiz" : `Chỉnh sửa câu hỏi — ${quizTitle}`)
          ...
          <button onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-all text-xl">×</button>
        </div>
          ...
          {step === 1 ? (
            <div className="space-y-4">
              ...
              <Field label="Tài liệu gắn với Quiz (Tuỳ chọn)">
                <select value={selectedDoc} onChange={(e) => setSelectedDoc(e.target.value)}
                  disabled={docsLoading} className={cls(false)}>
                  <option value="">-- Không gắn tài liệu --</option>
                  {docs.map((d) => <option key={d._id ?? d.id} value={d._id ?? d.id}>{d.title ?? d.name}</option>)}
                </select>
              ...
              <div className={`grid gap-3 ${!documentId ? "grid-cols-2" : ""}`}>
                {!documentId && (
                  <Field label="Độ khó">
                    <select value={difficulty} onChange={(e) => setDiff(e.target.value)} className={cls(false)}>
            ...
            <div className="space-y-4">
                ...
                <div className="border border-dashed border-gray-200 rounded-xl p-4 space-y-3 bg-gray-50/40">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {LETTERS.map((l) => {
                      ...
                      return (
                        <div key={l}>
                          <input type="text" name={key} placeholder={`Đáp án ${l}`} value={currentQ[key]}
                            onChange={handleQChange} className={cls(qErrors[key])} />
              ...
              {questions.length >= MAX_Q && editIdx === null && (
                <OrangeInfo>Đã đạt tối đa <strong>{MAX_Q} câu hỏi</strong>. Xoá bớt để thêm câu mới.</OrangeInfo>
              )}
