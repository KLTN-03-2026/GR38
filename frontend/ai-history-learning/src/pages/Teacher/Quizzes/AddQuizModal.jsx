...
const OrangeInfo = ({ children }) => (
  <div className="flex items-center gap-2 bg-orange-50 border border-orange-100 rounded-xl px-4 py-3">
    ...
    <p className="text-xs text-orange-600">{children}</p>
  </div>
);
...
function ThumbnailPicker({ preview, name, onChange, onRemove, error, maxMb }) {
  ...
  return (
    <Field label="Ảnh bìa (Thumbnail)" error={error}>
      {preview ? (
        <div
          className="relative w-full rounded-xl overflow-hidden border border-gray-200 group"
          style={{ height: 160 }}
        >
          <img
            src={preview}
          ...
          >
            <label className="cursor-pointer px-3 py-1.5 bg-white rounded-lg text-xs font-medium text-gray-700 hover:bg-gray-100">
              Đổi ảnh <FileInput />
            </label>
            <button
              type="button"
          ...
          {name && (
            <div className="absolute bottom-2 left-2 bg-black/50 backdrop-blur-sm rounded-md px-2 py-0.5">
              <span className="text-[10px] text-white truncate max-w-[200px] block">
                {name}
              </span>
            </div>
          )}
...
export default function AddQuizModal({
  ...
  useEffect(() => {
    ...
    // Ưu tiên documentId từ editQuiz, fallback về prop documentId
    const docId = editQuiz.documentId ?? documentId ?? "";
    setSelectedDoc(String(docId));
    setQuestions(
      (editQuiz.questions ?? []).map((q) => {
        const idx =
          q.correctAnswerIndex ?? q.options?.indexOf(q.correctAnswer) ?? 0;
        return {
          question: q.question,
          options: q.options ?? [],
  ...
  const handleThumbChange = (e) => {
    ...
    if (file.size > MAX_MB * 1024 * 1024) {
      setThumbErr(`Ảnh quá lớn, tối đa ${MAX_MB}MB`);
      return;
    }
    setThumbErr("");
    setThumb(file);
  ...
  const validateQ = () => {
    ...
    LETTERS.forEach((l) => {
      if (!currentQ[`option${l}`].trim())
        err[`option${l}`] = `Vui lòng nhập đáp án ${l}`;
    });
    if (!currentQ.correctAnswer)
      err.correctAnswer = "Vui lòng chọn đáp án đúng";
    setQErrors(err);
    return !Object.keys(err).length;
  };

  const handleAddQ = () => {
    if (editIdx === null && questions.length >= MAX_Q) {
      setListErr(`Tối đa ${MAX_Q} câu hỏi`);
      return;
    }
    if (!validateQ()) return;
    ...
    if (!options[ci]) {
      setQErrors((p) => ({ ...p, correctAnswer: "Đáp án đúng không hợp lệ" }));
      return;
    }
    const built = {
  ...
  const handleEdit = (i) => {
    ...
    setCurrentQ({
      question: q.question,
      ...Object.fromEntries(
        LETTERS.map((l, j) => [`option${l}`, q.options[j] ?? ""]),
      ),
  ...
  const handleFinish = async () => {
    if (questions.length < MIN_Q) {
      setListErr(`Cần ít nhất ${MIN_Q} câu hỏi`);
      return;
    }
    try {
      setSaving(true);
      ...
      setSaveErr(
        err?.response?.data?.message ||
          `Lỗi ${isEdit ? "cập nhật" : "tạo"} Quiz, vui lòng thử lại.`,
      );
    } finally {
      setSaving(false);
    }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100 shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-gray-800">
              {isEdit
                ? step === 1
                  ? "Chỉnh sửa Quiz"
                  : `Chỉnh sửa câu hỏi — ${quizTitle}`
                : step === 1
                  ? "Tạo Quiz mới"
                  : `Thêm câu hỏi — ${quizTitle}`}
            </h2>
            <div className="flex items-center gap-2 mt-1.5">
              {[1, 2].map((s) => (
                <div key={s} className="flex items-center gap-1.5">
                  ...
                  <span
                    className="text-[11px]"
                    style={{ color: step >= s ? "#F26739" : "#9ca3af" }}
                  >
                  ...
                  {s < 2 && (
                    <div
                      className="w-5 h-px"
                      style={{ background: step > s ? "#F26739" : "#e5e7eb" }}
                    />
                  )}
                </div>
              ))}
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-all text-xl"
          >
            ×
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
          {step === 1 ? (
            <div className="space-y-4">
              ...
              <div className={`grid gap-3 ${!documentId ? "grid-cols-2" : ""}`}>
                {!documentId && (
                  <Field label="Độ khó">
                    <select
                      value={difficulty}
            ...
            <div className="space-y-4">
              {questions.length > 0 && (
                <div className="space-y-1.5">
                  {questions.map((q, i) => (
                    ...
                    >
                      <span className="text-[11px] font-semibold text-gray-400 shrink-0 w-5">
                        #{i + 1}
                      </span>
                ...
                <div className="border border-dashed border-gray-200 rounded-xl p-4 space-y-3 bg-gray-50/40">
                  ...
                  <div>
                    <textarea
                      ...
                      rows={2}
                      className={`${cls(qErrors.question)} resize-none`}
                    />
                    {qErrors.question && (
                  ...
                  <div>
                    <select
                      ...
                      onChange={handleQChange}
                      className={cls(qErrors.correctAnswer)}
                    >
                  ...
                  <button
                    ...
                    onMouseEnter={(e) =>
                      (e.currentTarget.style.background = "#ffefe8")
                    }
                    onMouseLeave={(e) =>
                      (e.currentTarget.style.background = "#fff8f5")
                    }
              ...
              {questions.length >= MAX_Q && editIdx === null && (
                <OrangeInfo>
                  Đã đạt tối đa <strong>{MAX_Q} câu hỏi</strong>. Xoá bớt để
                  thêm câu mới.
                </OrangeInfo>
              )}
              {listErr && (
                <p className="text-xs text-red-500 text-center">{listErr}</p>
              )}
              {saveErr && (
        ...
        <div className="flex justify-end gap-2.5 px-6 pb-5 pt-3.5 border-t border-gray-100 shrink-0">
          <button
            onClick={onClose}
            className={`${btnBase} border border-gray-200 text-gray-600 ghost-btn`}
          >
            Huỷ
          </button>
          {step === 1 ? (
            <button
              onClick={() => {
                if (!quizTitle.trim()) {
                  setTitleErr("Vui lòng nhập tên Quiz");
                  return;
                }
                setStep(2);
              }}
              className={`${btnBase} text-white primary-btn`}
            ...
            <button
              ...
              disabled={saving}
              className={`${btnBase} text-white primary-btn disabled:opacity-60`}
              style={{ background: "#F26739" }}
            >
              {saving
                ? "Đang lưu..."
