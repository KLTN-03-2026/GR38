...
// ── Quiz Card ────────────────────────────────────────────────────────────────
function QuizCard({ quiz, isTeacher, onStart, onEdit, onDelete }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden"
      ...
      <div className="relative overflow-hidden">
        {quiz.coverImage ? (
          <img src={quiz.coverImage} alt={quiz.title} className="w-full h-40 object-cover bg-[#fdf3ec]" />
        ) : (
        ...
        {quiz.difficulty && (
          <span className={`absolute top-2.5 left-2.5 text-white text-[11px] px-2.5 py-1 rounded-full font-bold shadow-sm ${
            quiz.difficulty === "EASY" ? "bg-green-500" : quiz.difficulty === "MEDIUM" ? "bg-yellow-500" : "bg-red-500"
          }`}>
...
function ByDocumentTab({ isTeacher, onStartQuiz, onOpenAddModal, onOpenEditModal }) {
  ...
  const fetchQuizzes = useCallback(async (docId) => {
    ...
    try {
      ...
      setQuizzes(list.map((q) => ({
        ...
        questions: q.questions ?? [], coverImage: q.coverImage ?? null,
      })));
    } catch { setQuizError("Không thể tải danh sách quiz. Vui lòng thử lại."); }
    finally { setQuizLoading(false); }
  }, []);

  const handleSelectDoc = (id, title) => {
    setSelectedDocId(id);
  ...
  return (
    <div className="flex gap-5">
      ...
      <div className="w-64 shrink-0 bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col"
        ...
        <div className="px-4 pt-4 pb-3 border-b border-gray-100">
          ...
          <div className="flex items-center bg-gray-50 border border-gray-100 rounded-lg px-2.5 h-8 gap-2">
            <svg width="12" height="12" className="text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {docsLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <button onClick={() => navigate("/teacher/documents")}
                className="mt-2 text-xs text-orange-500 underline">Tạo tài liệu →</button>
            </div>
            ...
            return (
              <button key={id} onClick={() => handleSelectDoc(id, title)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {doc.createdAt && (
                    <p className="text-[10px] text-gray-400">{new Date(doc.createdAt).toLocaleDateString("vi-VN")}</p>
                  )}
                </div>
      ...
      <div className="flex-1 min-w-0">
        {!selectedDocId ? (
          <div className="flex flex-col items-center justify-center h-64 gap-3 text-gray-400 border-2 border-dashed border-gray-200 rounded-2xl">
            <svg width="32" height="32" fill="none" stroke="currentColor" viewBox="0 0 24 24" className="text-gray-300">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 19l-7-7 7-7" />
            </svg>
            <p className="text-sm text-gray-400">Chọn tài liệu bên trái để xem Quiz</p>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => fetchQuizzes(selectedDocId)} disabled={quizLoading}
                  ...
                  <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24" className={quizLoading ? "animate-spin" : ""}>
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredQuizzes.map((quiz) => (
                  <QuizCard key={quiz._id ?? quiz.id} quiz={quiz} isTeacher={isTeacher}
                    onStart={onStartQuiz} onEdit={onOpenEditModal}
                    onDelete={(q) => setDeleteTarget(q)} />
                ))}
              </div>
            )}

            {!quizLoading && quizzes.length > 0 && filteredQuizzes.length === 0 && (
              <div className="text-center py-20 text-gray-400 text-sm">Không tìm thấy quiz nào</div>
            )}
          </>
        )}
      </div>
...
// ── Tab: Thủ công ────────────────────────────────────────────────────────────
function ManualTab({ isTeacher, onStartQuiz, onOpenAddModal, onOpenEditModal, quizzes, onDeleteQuiz }) {
  const [deleteTarget, setDeleteTarget] = useState(null);
  const [deleting, setDeleting]         = useState(false);
    ...
    <div>
      ...
      {quizzes.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 gap-3 border-2 border-dashed border-gray-200 rounded-2xl">
          ...
          <p className="text-sm font-medium text-gray-500">Chưa có quiz thủ công nào</p>
          {isTeacher && (
            <button onClick={() => onOpenAddModal(null)}
...
export default function QuizPage() {
  ...
  // ✅ FIX 1: Đọc từ localStorage khi khởi tạo
  ...
  // ✅ FIX 2: Tự động lưu vào localStorage mỗi khi manualQuizzes thay đổi
  useEffect(() => {
      ...
      if (Array.isArray(quiz.questions) && quiz.questions.length > 0) {
        const normalized = quiz.questions.map((q) => ({
          ...
          answer:   q.correctAnswerIndex ??
                    (typeof q.correctAnswer === "number" ? q.correctAnswer : q.options?.indexOf(q.correctAnswer)) ??
                    q.answer ?? 0,
        }));
  ...
  const handleSaveQuiz = (result, isEdit, localQuestions = []) => {
    ...
    const finalQuestions = localQuestions.length > 0 ? localQuestions : (data.questions ?? []);
    const normalized     = {
      id: data._id ?? data.id, _id: data._id ?? data.id,
      title: data.title, description: data.description,
      difficulty: data.difficulty, time_limit: data.time_limit,
      questionCount: finalQuestions.length,
      questions: finalQuestions,
    };

    if (isEdit) {
      setManualQuizzes((prev) => prev.map((q) =>
  ...
  if (quizView) return (
    <>
      <GlobalStyles />
      <QuizView quiz={quizView.quiz} questions={quizView.questions}
        onBack={() => setQuizView(null)} onFinish={handleFinish} />
    </>
  );
          ...
          <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-2xl p-1 w-fit shadow-sm">
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold transition-all ${
                activeTab === "document" ? "text-white shadow-sm" : "text-gray-500 hover:text-gray-700"
              }`}
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold transition-all ${
                activeTab === "manual" ? "text-white shadow-sm" : "text-gray-500 hover:text-gray-700"
              }`}
              ...
              {manualQuizzes.length > 0 && (
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                  activeTab === "manual" ? "bg-white/30 text-white" : "bg-orange-100 text-orange-600"
                }`}>
