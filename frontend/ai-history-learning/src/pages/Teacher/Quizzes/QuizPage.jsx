  ...
  return (
    ...
    >
      <div className="relative overflow-hidden">
        {coverSrc ? (
          <img src={coverSrc} alt={quiz.title} className="w-full h-40 object-cover bg-[#fdf3ec]" />
        ) : (
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center bg-white border border-gray-200 rounded-xl px-3 h-9 gap-2 shadow-sm">
                  ...
                  <input type="text" placeholder="Tìm quiz..." value={quizSearch} onChange={(e) => setQuizSearch(e.target.value)}
                ...
                {isTeacher && (
                  <button onClick={() => onOpenAddModal(selectedDocId)} className="h-9 px-4 text-white text-xs font-semibold rounded-xl transition" style={{ background: "#F26739" }}>
                    + Thêm Quiz
                  </button>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredQuizzes.map((quiz) => (
                  <QuizCard key={quiz._id ?? quiz.id} quiz={quiz} isTeacher={isTeacher}
                    ...
                    onDelete={(q) => setDeleteTarget(q)} docThumbnail={selectedDocThumb} />
                ))}
              </div>
            )}
        ...
        <div className="flex items-center gap-2">
          {quizzes.length > 0 && (
            <div className="flex items-center bg-white border border-gray-200 rounded-xl px-3 h-9 gap-2 shadow-sm">
              ...
              <input type="text" placeholder="Tìm quiz..." value={search} onChange={(e) => setSearch(e.target.value)}
                className="bg-transparent border-none outline-none text-xs w-32 text-gray-700" />
            </div>
          )}
          {isTeacher && (
...
export default function QuizPage() {
  ...
  const handleFinish = ({ quiz, answers, score, total, questions, answered, resultId }) => {
    setQuizView(null);
    navigate("/teacher/quiz-result", { state: { quiz, answers, score, total, questions, answered, resultId } });
  };
      ...
      <div className="h-full overflow-y-auto bg-gray-50 px-8 py-6">
          ...
          <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-2xl p-1 w-fit shadow-sm">
            ...
            <button onClick={() => setActiveTab("manual")}
              ...
              {manualQuizzes.length > 0 && (
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${activeTab === "manual" ? "bg-white/30 text-white" : "bg-orange-100 text-orange-600"}`}>
        ...
        {activeTab === "document" && (
          <ByDocumentTab isTeacher={isTeacher} onStartQuiz={handleStartQuiz}
            onOpenAddModal={handleOpenAddModal} onOpenEditModal={setEditTarget} />
        )}
        {activeTab === "manual" && (
          <ManualTab
            isTeacher={isTeacher}
