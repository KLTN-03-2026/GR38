...
export default function QuizView({ quiz, questions, onBack, onFinish }) {
  ...
  const timeLimitSec = (quiz.time_limit ?? 0) * 60;
  ...
  const handleJump = (i) => {
    if (i !== currentQ) {
      if (answers[currentQ] === undefined && i > currentQ) { setShowWarn(true); return; }
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 overflow-hidden">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-3.5 flex items-center justify-between shrink-0">
        ...
        {/* Timer */}
        {timeLimitSec > 0 && (
          <div className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-bold transition-colors ${
            timeLeft <= 60 ? "bg-red-50 text-red-500 animate-pulse" : "bg-orange-50 text-orange-500"
          }`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-1 justify-center flex-wrap">
                      {pageNums.map((i, arrIdx) => (
                        <span key={`page-${arrIdx}`} className="flex items-center gap-1.5">
                    ...
                    {currentQ < total - 1 ? (
                      <button
                        onClick={handleNext}
                        className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white shrink-0 ${showWarn ? "next-btn-warn warn-shake" : selectedAnswer !== undefined ? "next-btn-ready" : ""}`}
